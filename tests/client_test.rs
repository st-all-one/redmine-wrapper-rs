//! Testes de integracao do cliente HTTP com wiremock.
...
async fn setup() -> (MockServer, RedmineClient) {
...
#[tokio::test]
async fn test_client_creation_fails_without_url() {
    let config = RedmineConfig {
        base_url: "".into(),
        token: Some("token".into()),
        ..Default::default()
...
#[tokio::test]
async fn test_client_creation_succeeds_without_token() {
    let config = RedmineConfig {
        base_url: "https://redmine.example.com".into(),
        token: None,
        ..Default::default()
...
async fn test_auth_header_sent() {
    ...
    let result = client.issues.list(None).await;
    assert!(result.is_ok());
}
...
async fn test_issues_list() {
    ...
    let result = client.issues.list(None).await.unwrap();
...
async fn test_issues_get() {
    ...
    let result = client.issues.get(1).await.unwrap();
...
async fn test_404_error() {
    ...
    let result = client.issues.get(999).await;
    assert!(result.is_err());
...
async fn test_validation_error() {
    ...
    let result = client.issues.create(&payload).await;
    assert!(result.is_err());
    let err = result.unwrap_err();
...
async fn test_projects_list() {
    ...
    let result = client.projects.list().await.unwrap();
    assert_eq!(result.len(), 1);
